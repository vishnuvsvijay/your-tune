...
import { FaPlay, FaHeart, FaRegHeart, FaEllipsisV, FaRandom, FaThumbsUp, FaRegThumbsDown } from 'react-icons/fa'
import { IoShuffleOutline } from 'react-icons/io5'

function Liked() {
  const { user } = useAuth()
  const { currentSong, playing, setCurrentSong, togglePlay } = usePlayer()
  ...
  useEffect(() => {
    ...
    const onLike = (msg) => {
      if (!user?.id) return
      // We refresh if the current user liked/unliked something
      // or if any song in our list was affected (though this is specifically for "Liked" page)
      load()
    }
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-white font-sans pb-24 overflow-y-auto custom-scrollbar">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Header Section */}
        <div className="flex flex-col md:flex-row items-center md:items-end gap-10 mb-10 bg-gradient-to-b from-[#4d1a4d]/40 to-transparent p-10 rounded-[40px]">
          {/* Large Cover Image */}
          <div className="relative group shrink-0">
            <div className="w-64 h-64 md:w-80 md:h-80 rounded-[32px] bg-gradient-to-br from-[#ff00ff] to-[#7f00ff] flex items-center justify-center shadow-[0_20px_50px_rgba(127,0,255,0.3)] overflow-hidden relative">
              <FaThumbsUp className="text-white text-[160px] drop-shadow-2xl" />
          ...
          <div className="flex-1 flex flex-col items-center md:items-start gap-6 text-center md:text-left">
            <div>
              <h1 className="text-6xl md:text-8xl font-black mb-4 tracking-tighter italic">Liked music</h1>
            ...
            <div className="flex flex-col gap-1 text-gray-400 font-bold">
              <p>Auto playlist • {new Date().getFullYear()}</p>
              ...
              <p className="max-w-md mt-4 text-sm leading-relaxed opacity-60">
                your liked songs are here for you
              </p>
            </div>
        ...
        <div className="mt-12 px-4">
          {loading ? (
            ...
            <div className="text-center py-32 bg-white/5 rounded-[40px] border-2 border-dashed border-white/5">
              <FaThumbsUp className="text-8xl mx-auto mb-6 opacity-10" />
              <h3 className="text-2xl font-black italic mb-2">No liked music yet</h3>
              <p className="text-gray-500 font-medium">Songs you like will appear here for easy access.</p>
            </div>
          )}
        </div>
      </div>
